...
import { Mail, Lock, Zap, GitBranch, Info } from 'lucide-react'
...
function initiateOAuth(provider: 'google' | 'github') {
  ...
  if (provider === 'google') {
    ...
    if (!clientId) { alert('Google OAuth not configured.'); return }
    ...
    if (!clientId) { alert('GitHub OAuth not configured.'); return }
    const params = new URLSearchParams({ client_id: clientId, redirect_uri: redirectUri, scope: 'user:email' })
...
export default function LoginPage() {
  ...
  // When redux signals MFA is needed, redirect to the MFA page
  useEffect(() => {
    if (mfaRequired) navigate('/mfa-verify', { replace: true })
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    if (loginThunk.fulfilled.match(result) && !result.payload.requiresMfa) {
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Input
              ...
              leftIcon={<Mail size={14} />} error={errors.email?.message}
              {...register('email')}
            />
            <Input
              label="Password" type="password" placeholder="••••••••"
              leftIcon={<Lock size={14} />} error={errors.password?.message}
              {...register('password')}
            />
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" type="button" className="gap-2 text-xs" onClick={() => initiateOAuth('google')}>
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                  <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" />
                </svg>
                Google
              </Button>
              <Button variant="outline" type="button" className="gap-2 text-xs" onClick={() => initiateOAuth('github')}>
